...
function moove_activity_get_referrer_link_by_url( $url ) {
	...
	$return = '';
	if ( $post_id ) : ?>
		<a href="<?php the_permalink( $post_id ); ?>" target="_blank"><?php echo get_the_title( $post_id ); ?></a>
	<?php else : ?>
		<?php if ( strpos( $url, 'wp-admin/post') ) :
		 ...
		 ?>
			<a href="<?php echo $edit_url; ?>" target="_blank"><?php echo $page_name; ?></a>
		<?php else : ?>
...
function moove_activity_get_timezone_dropdown( $selected ) {
    ...
    <select name="timezone">
        <?php foreach( $list as $t ) : ?>
          <option value="<?php echo $t['zone']; ?>" <?php echo $selected === $t['zone'] ? 'selected="selected"' : ''; ?>>
            <?php echo $t['zone']; ?>
          </option>
        <?php endforeach; ?>
    </select>
