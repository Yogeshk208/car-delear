...
class Moove_Activity_Options {
			...
			if ( $activity_settings[ $post_type ] === '1' || $activity_settings[ $post_type ] === 1 ) :
				...
				if ( have_posts() ) :
					while ( have_posts() ) : the_post();
						global $post;
						Moove_Activity_Content::moove_save_post( $post->ID, 'enable' );
					endwhile;
	...
	function moove_activity_settings_page() {
		...
		echo Moove_Activity_View::load( 'moove.admin.settings.settings_page', array() );
	}
	...
	function moove_get_logs_count( $post_type ) {
		// $posts = Moove_Activity_Database_Model::get_log( '`post_type`', $post_type );
		$posts = array();
		return count( $posts );
	}
	/**
	...
	function moove_activity_page_init() {
		...
		add_settings_section(
			'post_type_act', // ID.
			'Post-type activity tracking', // Title.
		...
		foreach ( $post_types as &$post_type ) :
			add_settings_field(
				...
				array(
					'post_type' 	=> $post_type,
					'post_count' 	=> Moove_Activity_Options::moove_get_logs_count( $post_type )
				)
			);
		endforeach;
	}
	...
	function moove_activity_print_section_info() {
		return _e( 'This page provides a facility to set activity log options globally for post types.' , 'moove' );
	}
	/**
